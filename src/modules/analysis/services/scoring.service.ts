  ...
  static calculateReviewDepthScore(prMetadata: {
    ...
    if (prMetadata.isMerged && prMetadata.mergedAt) {
      const timeToMergeMinutes =
        (prMetadata.mergedAt.getTime() - prMetadata.createdAt.getTime()) /
        (1000 * 60);
      ...
      const totalComments =
        prMetadata.commentsCount + prMetadata.reviewCommentsCount;
      if (totalComments === 0) {
      ...
      if (
        prMetadata.author === prMetadata.mergedBy &&
        prMetadata.requestedReviewersCount === 0 &&
        totalComments === 0
      ) {
        score -= 100;
      }
      ...
      if (
        prMetadata.requestedReviewersCount === 0 &&
        prMetadata.commentsCount + prMetadata.reviewCommentsCount === 0
      ) {
        score -= 50;
      }
    }
