...
export default async function RepositoryViewPage({ 
    ...
    return (
      <main className={styles.main}>
        <div className={styles.container}>
          ...
          <Link href="/dashboard/repos" style={{ color: 'var(--muted)' }}>&larr; Back to repositories</Link>
        </div>
      </main>
    );
  }
  ...
  const [totalFindings, findings] = await Promise.all([
    prisma.finding.count({ where: { pullRequest: { repositoryId: repository.id } } }),
    ...
    <div>
      <div style={{ marginBottom: 'var(--space-8)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 'var(--space-4)' }}>
          <div>
            <h1 className="text-h1" style={{ marginBottom: 'var(--space-1)' }}>{repository.name}</h1>
      ...
      <section>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--color-border)', paddingBottom: 'var(--space-2)', marginBottom: 'var(--space-4)' }}>
          ...
          <span className="text-small">Showing {findings.length} of {totalFindings}</span>
        </div>

        {findings.length === 0 ? (
          <div className="panel" style={{ padding: 'var(--space-12) var(--space-4)', textAlign: 'center' }}>
            <p className="text-muted" style={{ margin: '0 0 var(--space-2) 0' }}>No findings generated yet.</p>
            <p className="text-small" style={{ margin: 0 }}>Findings will appear here as the system analyzes new commits and pull requests.</p>
          </div>
        ) : (
              ...
              return (
                <div key={finding.id} style={{ 
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 'var(--space-3)' }}>
                    <div>
                      <h3 className="text-h3" style={{ margin: '0 0 var(--space-1) 0' }}>
                        <Link href={`/dashboard/findings/${finding.id}`} style={{ color: 'var(--color-foreground)' }}>
                          {sourceTitle}
                        </Link>
                      </h3>
                      <p className="text-small" style={{ margin: 0 }}>
                        Author: {author} • Date: <span suppressHydrationWarning>{new Date(finding.createdAt).toLocaleDateString()}</span>
                      </p>
                    </div>
                  ...
                  <p className="text-body" style={{ margin: 'var(--space-2) 0 0 0', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                    AI analysis evaluated this pull request with a depth score of {finding.reviewDepthScore}. Requires review.
                  </p>
